...
"""
VOTE 17 — HELD-STATE RATIFICATION: is OG Coil 1 HELD at N=415?

Written BEFORE the D1/D2 re-run verdict was computed (2026-08-30), so the
framing cannot be tuned to the outcome. Re-run under declarations D1+D2 (journal 2026-08-30,
declared before any data): statistics computed on non-settling samples only;
rule (c) threshold = 1.5x the sd of the DURATION-MATCHED (60-min) detrended
servo-off control. Rules: |mean delta| < 0.5 deg, |trend| < 1.0 deg/hour,
sd(non-settling) <= 1.5x matched control sd.
...
Sealed to redis cadence:tworocks:next-vote-17 with the mechanical window
...
print("VOTE 17 — HELD-STATE RATIFICATION (framed before the verdict existed)")
...
R.set("cadence:tworocks:next-vote-17", json.dumps({
...
print(" sealed: cadence:tworocks:next-vote-17")
